...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $usertype = 'customer';
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => [], ,'totaljobs' => 0, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => 0, 'pagenum' => 0];
        } elseif ($cuser && $cuser->is('translator')) {
            ...
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'totaljobs' => $totaljobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum];
        }
    }
    ...
    {
        $immediatetime = 5;
        $consumer_type = isset($user->userMeta->consumer_type)?$user->userMeta->consumer_type:'';
        if (isset($user->user_type) && $user->user_type == env('CUSTOMER_ROLE_ID')) {
            $cuser = $user;
            ...
            if (isset($due) && isset($data['b_created_at']))
            ...
            if($job){
                ...
                $data['job_for'] = array();
                if (isset($job->gender) && $job->gender != null) {
                    if ($job->gender == 'male') {
                        $data['job_for'][] = 'Man';
                ...
                if (isset($job->certified) && $job->certified != null) {
                ...
                $data['customer_town'] = isset($cuser->userMeta->city)?$cuser->userMeta->city:'';
                $data['customer_type'] = isset($cuser->userMeta->customer_type)?$cuser->userMeta->customer_type:'';
                ...
                $response['status'] = 'fail';
                $response['message'] = "Translator can not post new job";
            }
        } else {
            $response['status'] = 'fail';
    ...
    public function storeJobEmail($data)
    {
        ...
        if(!isset(@$data['user_email_job_id'])){
            $response['status'] = 'fail';
            $response['message'] = "Email job id is mising";
            return $response;
        }
    ...
    public function jobToData($job)
    {
        ...
        $data = array();            
        $data['job_id'] = isset($job->id)?$job->id:'';
        $data['from_language_id'] = isset($job->from_language_id)?$job->from_language_id:'';
        $data['immediate'] = isset($job->immediate)?$job->immediate:'';
        $data['duration'] = isset($job->duration)?$job->duration:'';
        $data['status'] = isset($job->status)?$job->status:'';
        $data['gender'] = isset($job->gender)?$job->gender:'';
        $data['certified'] = isset($job->certified)?$job->certified:'';
        $data['due'] = isset($job->due)?$job->due:'';
        $data['job_type'] = isset($job->job_type)?$job->job_type:'';
        $data['customer_phone_type'] = isset($job->customer_phone_type)?$job->customer_phone_type:'';
        $data['customer_physical_type'] = isset($job->customer_physical_type)?$job->customer_physical_type:'';
        $data['customer_town'] = isset($job->town)?$job->town:'';
        $data['customer_type'] = (isset($job->user) && isset($job->user->userMeta->customer_type))?$job->user->userMeta->customer_type:'';
        ...
        $data['job_for'] = array();
        if (isset($job->gende) && $job->gender != null) {
            if ($job->gender == 'male') {
                $data['job_for'][] = 'Man';
            } else if ($job->gender == 'female') {
                $data['job_for'][] = 'Kvinna';
    ...
    public function jobEnd($post_data = array())
    {
        ...
        if(isset($post_data["job_id"])){
            ...
            if(isset($tr->user_id) && isset($job->user_id))
            ...
            $tr->completed_by = isset($post_data['userid'])?$post_data['userid']:'';
            $tr->save();
        }
    }
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $translator_type = isset($user_meta->translator_type)?$user_meta->translator_type:'';
        $job_type = 'unpaid';
        ...
        $gender = isset($user_meta->gender)?$user_meta->gender:'';
        $translator_level = isset($user_meta->translator_level)?$user_meta->translator_level:'';
        ...
        if($job_ids){
            foreach ($job_ids as $k => $v)     // checking translator town
            {
                ...
                if(isset($job->customer_phone_type) && isset($job->customer_physical_type)){
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $msg_contents = '';
        if(isset($data['immediate']) && isset($data['language']) && isset($data['duration']) && isset($data['due'])){
            if ($data['immediate'] == 'no') {
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $jobPosterMeta = UserMeta::where('user_id', isset($job->user_id)?$job->user_id:'')->first();

        // prepare message templates
        $date = isset($job->due)?date('d.m.Y', strtotime($job->due)):date('d.m.Y');
        $time = isset($job->due)?date('H:i', strtotime($job->due)):date('H:i');
        ...
        if(isset($job->customer_physical_type)){}
        ...
        if($translators){
            foreach ($translators as $translator) {
                ...
                if(isset($translator->email) && isset($translator->mobile)){
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $joblanguage = isset($job->from_language_i)?$job->from_language_id:'';
        $gender = isset($job->gender)?$job->gender:'';
        $translator_level = [];
        if (isset($job->certified) && !empty($job->certified)) {
        ...
        $blacklist = UsersBlacklist::where('user_id', isset($job->user_id)?$job->user_id:'')->get();
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        if ($changeTranslator['translatorChanged'] && $changeTranslator['log_data']) $log_data[] = $changeTranslator['log_data'];
        ...
        if (isset($job->from_language_id) && isset($data['from_language_id']) && $job->from_language_id != $data['from_language_id']) {
            $log_data[] = [
        ...
        if(isset($changeTranslator['translatorChanged'])){
            $changeStatus = $this->changeStatus($job, $data, $changeTranslator['translatorChanged']);
            if ($changeStatus['statusChanged'])
                $log_data[] = $changeStatus['log_data'];
        }

        $job->admin_comments = isset($data['admin_comments'])?$data['admin_comments']:'';
    ...
    {
        $old_status = isset($job->status)?$job->status:'';
        $statusChanged = false;
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        $old_status = isset($job->status)?$job->status:'';
        $job->status = isset($data['status'])?$data['status']:'';
        $user = $job->user()->first();
        if (isset($job->user_email) && !empty($job->user_email)) {
            $email = $job->user_email;
        } else {
        ...
        if (isset($data['status']) && $data['status'] == 'pending') {
            ...
            if($job){
                if($job->id && $job->from_language_id)
                    $subject = 'Vi har nu återöppnat er bokning av ' . TeHelper::fetchLanguageFromJobId($job->from_language_id) . 'tolk för bokning #' . $job->id;
                else
                    $subject = 'Vi har nu återöppnat er bokning';
                if(!empty($email))
            ...
            if($job){
                if($job->id)
                    ...
                    $subject = 'Bekräftelse - tolk har accepterat er bokning';
                if(!empty($email))
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'completed'])) {
        $job->status = isset($data['status'])?$data['status']:'';
        if (!isset($data['admin_comments']) || $data['admin_comments'] == '') return false;
        $job->admin_comments = $data['admin_comments'];
        if (isset($data['status']) && $data['status'] == 'completed') {
            $user = $job->user()->first();
            if (!isset($data['sesion_time']) || $data['sesion_time'] == '') return false;
            ...
            if($job->id)
                ...
                $subject = 'Information om avslutad tolkning';
            if(!empty($email))
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        $job->status = isset($data['status'])?$data['status']:'';
        if ((!isset($data['admin_comments']) || $data['admin_comments'] == '') && (isset($data['status']) && $data['status'] == 'timedout')) return false;
        $job->admin_comments = $data['admin_comments'];
        $user = $job->user()->first();
        if (isset($job->user_email) && !empty($job->user_email)) {
            $email = $job->user_email;
        ...
        if (isset($data['status']) && $data['status'] == 'assigned' && $changedTranslator) {
            ...
            if($translator->email && $translator->name && !empty($translator->email))
                $this->mailer->send($translator->email, $translator->name, $subject, 'emails.job-changed-translator-new-translator', $dataEmail);
            if($job->from_language_id)
                $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
            if($user && $job->due && $job->duration)
                $this->sendSessionStartRemindNotification($user, $job, $language, $job->due, $job->duration);
            if($translator && $job->due && $job->duration)
...
//        }
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if (isset($job->customer_physical_type) && $job->customer_physical_type == 'yes')
            $msg_text = array(
        ...
        if ($this->bookingRepository->isNeedToSendPush($user->id)) {
            $users_array = ($user)?array($user):array();
            if(!empty($users_array) && isset($job->id) && isset($user->id))
    ...
    {
        if (isset($data['status']) && in_array($data['status'], ['timedout'])) {
            $job->status = $data['status'];
            if (!isset($data['admin_comments']) || $data['admin_comments'] == '') return false;
            $job->admin_comments = $data['admin_comments'];
            $job->save();
    ...
    {
        if (isset($data['status']) && in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            $job->status = $data['status'];
            if (!isset($data['admin_comments']) || $data['admin_comments'] == '' && $data['status'] == 'timedout') return false;
            $job->admin_comments = $data['admin_comments'];
            if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24'])) {
                ...
                if (isset($job->user_email) && !empty($job->user_email)) {
                    ...
                    $email = isset($user->user_email)?$user->email:'';
                }
                $name = isset($user->name)?$user->name:'';
                $dataEmail = [
                    'user' => $user,
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        if(isset($translator->email) && isset($translator->name) && !empty($translator->email))
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        if($job_ids && !empty($job_ids)){
            foreach ($job_ids as $k => $job) {
                ...
                if(isset($job->specific_job) && $job->specific_job == 'SpecificJob')
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    {
        $completeddate = date('Y-m-d H:i:s');
        $jobid = isset($post_data["job_id"])?$post_data["job_id"]:'';
        $job_detail = Job::with('translatorJobRel')->find($jobid);
        ...
        if($post_data['user_id'] && $job->user_id && $tr->user_id)
        ...
        $tr->completed_at = $completeddate;
        $tr->completed_by = isset($post_data["user_id"])?$post_data["user_id"]:'';
        $tr->save();
        $response['status'] = 'success';
        return $response;
    }
    ...
    public function alerts()
    {
        ...
        $cuser = Auth::user();
        $consumer_type = isset($cuser->id)?TeHelper::getUsermeta($cuser->id, 'consumer_type'):'';
    ...
    {
        $jobid = isset($request['jobid'])?$request['jobid']:'';
        ...
        if(isset($job['due']) && isset($data['created_at']))
            $data['will_expire_at'] = TeHelper::willExpireAt($job['due'], $data['created_at']);
        else
            $data['will_expire_at'] = date('Y-m-d H:i:s');
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        if (isset($job['status']) && $job['status'] != 'timedout') {
            ...
            $job['will_expire_at'] = isset($job['due'])?TeHelper::willExpireAt($job['due'], date('Y-m-d H:i:s')):date('Y-m-d H:i:s');
